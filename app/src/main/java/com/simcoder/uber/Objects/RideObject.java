...
public class RideObject  implements Cloneable{
    ...
    private String id ="";
    private float rideDistance = 100;
  ...
  //  DatabaseReference rideRef;
    ...
    private Double ridePrice = 20.0;
    private float calculatedRideDistance, timePassed = 10;
    ...
    public void parseData(DataSnapshot dataSnapshot){
      ...
      //  if(dataSnapshot.child("pickup").child("name").getValue()!=null){
            pickup.setName(/*ataSnapshot.child(*/"Haram"/*).child("name").getValue().toString()*/);
       // }
      //  if(dataSnapshot.child("destination").child("name").getValue()!=null){
            destination.setName(/*dataSnapshot.child(*/"Naser city"/*).child("name").getValue().toString()*/);
  //      }
    //    if(dataSnapshot.child("pickup").child("lat").getValue()!=null && dataSnapshot.child("pickup").child("lng").getValue()!=null){
            pickup.setCoordinates(
                    new LatLng(/*Double.parseDouble(dataSnapshot.child("pickup").child("lat").getValue().toString()*/0,
                              /*  Double.parseDouble(dataSnapshot.child("pickup").child("lng").getValue().toString()))*/0));
     //   }
    //    if(dataSnapshot.child("destination").child("lat").getValue()!=null && dataSnapshot.child("destination").child("lng").getValue()!=null){
            destination.setCoordinates(
                    new LatLng(0,0));/*Double.parseDouble(dataSnapshot.child("destination").child("lat").getValue().toString()),
   ...
   //     }
    ...
    //    }
     //   if(dataSnapshot.child("ended").getValue() != null){
            ended = false;//fBoolean.parseBoolean(dataSnapshot.child("ended").getValue().toString());
      //  }
      //  if(dataSnapshot.child("cancelled").getValue() != null){
            cancelled = false;// Boolean.valueOf(dataSnapshot.child("cancelled").getValue().toString());
      //  }
      //  if(dataSnapshot.child("cancelled_info").child("type").getValue() != null){
            cancelled = true;
            cancelledType =0; //Integer.parseInt(dataSnapshot.child("cancelled_info").child("type").getValue().toString());
      //  }
    //    if(dataSnapshot.child("cancelled_info").child("reason").getValue() != null){
     ...
     //   if(dataSnapshot.child("state").getValue() != null){
            state = 1;//Integer.parseInt(dataSnapshot.child("state").getValue().toString());
      //  }
    //    if(dataSnapshot.child("timestamp").getValue() != null){
            timestamp =2000L;// Long.valueOf(dataSnapshot.child("timestamp").getValue().toString());
    //    }
  //      if(dataSnapshot.child("price").getValue() != null){
            ridePrice = 200.0;//Double.valueOf(dataSnapshot.child("price").getValue().toString());
    //    }
      //  if(dataSnapshot.child("car").getValue() != null){
            car = "uuu-999";//dataSnapshot.child("car").getValue().toString();
      //  }
   ...
   //     if(dataSnapshot.child("rating").getValue() != null){
            rating = 2;//Integer.parseInt(dataSnapshot.child("rating").getValue().toString());
    //    }

   //     if(dataSnapshot.child("calculated_duration").getValue() != null){
            duration =100;// Double.parseDouble(dataSnapshot.child("calculated_duration").getValue().toString());
    ///    }
    //    if(dataSnapshot.child("calculated_distance").getValue() != null){
            distance = 2000;//Double.parseDouble(dataSnapshot.child("calculated_distance").getValue().toString());
    //    }
...
/*        if(id == null){return;}
    ...
    public void setStatus(int timePassed) {
 ...
 /*   public DatabaseReference getRideRef() {
    ...
    public static RideObject getRide(){
        RideObject ride = new RideObject();
        ride.setRideDistance(200);
        ride.setCustomer(CustomerObject.getCustomer());
        ride.setDriver(DriverObject.getDriver());
        ride.setPickup(LocationObject.getUserLocation());
        ride.setDestination(LocationObject.getDestination());
        ride.setCurrent(LocationObject.getDriverStartLocation());
        ride.estimatedPrice = 300;
        ride.duration = 300;
        ride.customerPaid =true;
        ride.distance =1000;
        ride.driverPaid =true;
        ride.timestamp =1000L;
        ride.calculatedRideDistance = 300;
        ride.calculatedRideDistance = 300;
        ride.state = 1;
        return ride;
    }
